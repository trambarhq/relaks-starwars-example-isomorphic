...
`ReactDOM.hydrate()` is used to render the application element into an already populated DOM node instead of `ReactDOM.render()`. See the [React documentation](https://reactjs.org/docs/react-dom.html#hydrate) for more details.
...
`ClientApp` is the SSR build of our app. After we've harvested the HTML tree, we pass it to [`ReactDOMServer.renderToString()`](https://reactjs.org/docs/react-dom-server.html#rendertostring). We then stick the resulting HTML into `index.html` and send it to the browser.
