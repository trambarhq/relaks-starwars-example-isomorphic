...
`FrontEnd` is the SSR build of our front-end. After we've harvested the HTML tree, we pass it to [`ReactDOMServer.renderToString()`](https://reactjs.org/docs/react-dom-server.html#rendertostring). We then stick the resulting HTML into `index.html` and send it to the browser.
